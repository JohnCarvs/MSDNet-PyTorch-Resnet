...
class ResNet_SDN_GE(nn.Module):
    def __init__(self, params):
        super(ResNet_SDN_GE, self).__init__()
        print("GGEGEGEGEG")
        self.num_blocks = params['num_blocks']
        self.num_classes = int(params['num_classes'])
        self.augment_training = params['augment_training']
        ...
        fwd = self.end_layers(fwd)
        outputs.append(fwd)
        print("output:{0}".format(len(outputs)))
    def _make_layer(self, channels, block_id, stride):
        num_blocks = int(self.num_blocks[block_id])
        strides = [stride] + [1] * (num_blocks - 1)
        layers = []
    ...
    def forward(self, x):
        ...
        for layer in self.layers:
            if idx in self.outputlayeridx:
                # print(idx)
                fwd = gradient_rescale(fwd, 1.0 / (self.nBlocks - exit_count))
                fwd, is_output, output = layer(fwd)
                fwd = gradient_rescale(fwd, (self.nBlocks - exit_count - 1))
                exit_count+=1
            else:
                fwd, is_output, output = layer(fwd)
            if is_output:
